...
function sanitizeVisibleSections(sections) {
  ...
  // When the user provides their own sections object, only keep
  // their boolean values — do not seed defaults underneath, so the
  // resulting object reflects exactly what the user controls.
  const out = {};
...
/**
 * Sanitize a full portfolio record (the shape accepted by
 * portfolioRepository.createOrUpdate).  Returns a new object with
 * all string fields HTML-stripped, all URL fields validated against
 * an https?:// allowlist, and all JSONB arrays capped.
 *
 * Always treats input as untrusted.  Pass-through fields not listed
 * below are dropped.
 */
...
/**
 * Defense-in-depth: sanitize a portfolio record that was read back
 * from the database.  Same rules as the write path, but operates on
 * a row that may have been written by older code.
 */
...
/**
 * Validate a single URL for use in href attributes.  Exposed for
 * tests and for the frontend.
 */
